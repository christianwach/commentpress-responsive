...
function commentpress_setup( 
	...
	/** 
	 * Make CommentPress Responsive Theme available for translation.
	...
	if ( count( $_data ) > 0 ) {
		...
		foreach ($_data as $comment) {
			...
			if ( $_title != $comment->post_title ) {
				...
				// show it
				$_page_content .= '<h4>'.$comment->post_title.' <span>('.$comment->comment_count.' '.$_comment_count_text.')</span></h4>'."\n\n";
